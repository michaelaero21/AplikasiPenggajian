        ...
        <tbody>
            @forelse($karyawans as $karyawan)
                <?php
                    ...
                    $kategoriFilter = strtolower($kategori ?? 'mingguan');

                    $slip = $karyawan->slipGaji->first(function ($item) use ($periode, $kategoriFilter) {
                        return $item->kategori_gaji === $kategoriFilter && $item->periode === $periode;
                    });
                ?>
                <tr data-id="{{ $karyawan->id }}"
                     data-slip="{{ $slip?->id ? 'yes' : 'no' }}"
                        data-slipid="{{ $slip?->id }}">
                     
                     {{-- === checkbox per baris === --}}
                     <td>
                    <input type="checkbox" class="sub_chk" value="{{ $karyawan->id }}"  data-slip="{{ $slip ? 'yes' : 'no' }}"
...
const updateToolbar = () => {
    // ambil kondisi mutakhir setiap kali
    ...
    if (breakdownEl) {
        breakdownEl.textContent =
            `(${belumSlip} siap generate · ${sudahSlip} ada slip)`;
    }
